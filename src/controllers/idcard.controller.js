...
function idCardData(user) {
  return {
    user: user.toJSON(),
    verifyUrl: `${process.env.API_BASE_URL || 'http://localhost:3000'}/api/verify/${user.id}`,
  };
}
...
const getQrCode = asyncHandler(async (req, res) => {
  ...
  const dataUrl = await QRCode.toDataURL(`${process.env.API_BASE_URL || 'http://localhost:3000'}/api/verify/${user.id}`);
